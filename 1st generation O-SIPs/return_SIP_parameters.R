...
for (SIP_library in list_of_SIP_libraries) {
  ...
  excluded_values <- eval(parse(text = exclude_value_name))
  ...
  #remove unconverged/undesired data points from SIP library
  cleaned.data <- exclude_list_of_distances(raw.data, excluded_values)
  
  #calculate the SIE of the SIP library
  cleaned.data$SIE <- calculate_SIE(cleaned.data)
  
  #fit the data to splines
  fitted_exponents <- exponent_fit_to_spline(cleaned.data, degrees_of_freedom)
  fitted_coefficients <- coefficient_fit_to_spline(cleaned.data, degrees_of_freedom)
  ...
  write.csv(parameterised_user_file.data,paste0(desired_SIP_library,"_output_",user_supplied_data_name, ".csv"))
